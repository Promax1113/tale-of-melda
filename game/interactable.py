...
class Interactable(pygame.sprite.Sprite):
    def __init__(self,texture_name: str, position: tuple, contents: list, _id: int) -> None:
        super().__init__()
        self.sprite_list = {}
        for texture in os.listdir(f"{os.getcwd()}/game_data/sprites/map/"):
            if texture_name in texture:
                ...
                self.sprite_list[texture.split("_")[1]] = pygame.transform.scale(img,(48,48))
...
class Chest(Interactable):
    def __init__(self,controller, texture_name: str, position: tuple, contents: list, trap: bool = False, _id: int = None) -> None:
    ...
    def update(self):
        if self.is_animating:
            ...
            if elapsed > self.animation_speed:
                ...
                if self.trap:
                    frame_index = min(int(frame_progress * len(self.sprite_list)), len(self.sprite_list) - 1)
                else:
                    frame_index = min(int(frame_progress * len(self.sprite_list)-1), len(self.sprite_list) - 1)
